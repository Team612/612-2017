...
void IntakeTest::Execute() {
  //CHANGE THE CHANNELS FOR THE GetCurrent METHODS TO THE REAL ONES
    if(RobotMap::pdp->GetCurrent(1) >= 35){
        std::cout << "THE INTAKE MOTOR IS DRAWING WAY TO MUCH POWER // LEFT INTAKE MOTOR"<< set::endl;
    ...
    if(RobotMap::pdp->GetCurrent(2) >= 35){
        std::cout << "THE INTAKE MOTOR IS DRAWING WAY TO MUCH POWER // RIGHT INTAKE MOTOR" << set::endl;
        Robot::oi->getgunner()->setRumble(frc::GenericHID::kRightRumble,1);
        Robot::oi->getgunner()->setRumble(frc::GenericHID::kLeftRumble,1);
    }
